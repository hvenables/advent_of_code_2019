...
def make_move(input)
  ...
  when 'R'
    @position[1].upto(@position[1] + (spaces - 1)).each do |y|
      @moves += 1
      @base_map[@position[0]][y] = if @base_map[@position[0]][y] == '.'
                                     "#{@moves}#{@wire}"
                                   else
                                     distance = (@moves + @base_map[@position[0]][y][0..-3].to_i) - 2
                                     @lowest = distance if distance < @lowest && distance != 0
                                     p distance
                                     distance
                                   end
    end
  ...
  when 'U'
    @position[0].upto(@position[0] + (spaces - 1)).each do |x|
      @moves += 1
      @base_map[x][@position[1]] = if @base_map[x][@position[1]] == '.'
                                     "#{@moves}#{@wire}"
                                   else
                                     distance = (@moves + @base_map[x][@position[1]][0..-3].to_i) - 2
                                     @lowest = distance if distance < @lowest && distance != 0
                                     p distance
                                     distance
                                   end
...
# puts @base_map.map { |x| x.map { |a| a.center(6) }.join(' ') }
